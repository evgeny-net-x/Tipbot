...
#[near_bindgen]
impl NearTips {
    pub(crate) fn asset_chat_owner(&self, chat_id: ChatId) {
    ...
    pub(crate) fn distribute_tiptokens(&mut self, chat_id: ChatId, chat_admin_account_id: AccountId, token_id: TokenAccountId, treasure_fee: Balance, sender_account_id: AccountId) {
    ...
    #[payable]
    pub fn claim_tiptokens_for_chat(&mut self, chat_id: ChatId, token_id: Option<TokenAccountId>) -> Promise {
        assert_one_yocto();
    ...
    pub fn get_chat_tokens(&self, chat_id: ChatId, token_id: Option<TokenAccountId>) -> WrappedBalance {
        let settings = self.get_chat_settings(chat_id);
        assert!(settings.is_some(), "Unknown chat");
    ...
    pub fn get_chat_points(&self, chat_id: ChatId) -> RewardPoint {
        self.chat_points.get(&chat_id).unwrap_or(0)
    }

    pub fn get_contacts_in_chats(&self, contact: Contact,
                                       chat_id: ChatId) -> bool {
        self.contacts_in_chats.contains(&ContactInChat {
            contact,
            chat_id,
        })
    }
    ...
    pub fn delete_chat_settings(&mut self, chat_id: ChatId) {
        self.assert_master_account_id();
        self.chat_settings.remove(&chat_id);
    }

    pub fn get_chat_settings(&self, chat_id: ChatId) -> Option<ChatSettings> {
        self.chat_settings.get(&chat_id)
    }

    pub fn get_chat_numerator(&self, chat_id: ChatId) -> TreasureFeeNumerator {
        let settings = self.get_chat_settings(chat_id);

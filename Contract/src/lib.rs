...
pub type ContactCategory = String;
...
pub struct NearTips {
    ...
    chat_settings: LookupMap<ChatId, ChatSettings>,
    treasure: LookupMap<TokenAccountId, Balance>,
    chat_points: LookupMap<ChatId, RewardPoint>,
...
#[derive(BorshDeserialize, BorshSerialize)]
pub struct TokenByContact {
    pub contact: Contact,
    pub token_account_id: TokenAccountId,
}

#[derive(BorshDeserialize, BorshSerialize)]
pub struct TokenByChat {
    pub chat_id: ChatId,
    pub token_account_id: TokenAccountId,
}

#[derive(BorshDeserialize, BorshSerialize)]
pub struct ContactInChat {
    pub contact: Contact,
    pub chat_id: ChatId,
}
...
pub trait ExtNearTips {
    ...
    fn on_withdraw_linkdrop(&mut self, amount: Balance, contact: Contact, public_key: String) -> bool;
    ...
    fn on_get_contact_owner_on_withdraw_with_auth(&mut self, #[callback] account: Option<AccountId>, recipient_account_id: AccountId, contact: Contact, token_id: Option<TokenAccountId>) -> bool;
    fn on_get_contact_owner_on_send_tip_with_auth(&mut self, #[callback] account: Option<AccountId>, sender_account_id: AccountId, tip_amount: Balance, contact: Contact, chat_id: Option<ChatId>, token_id: Option<TokenAccountId>) -> bool;

    fn after_ft_transfer_balance(&mut self, contact: Contact, amount: WrappedBalance, token_account_id: TokenAccountId) -> bool;
    fn after_ft_transfer_deposit(&mut self, account_id: AccountId, amount: WrappedBalance, token_account_id: TokenAccountId) -> bool;
    fn after_ft_transfer_claim_by_chat(&mut self, chat_id: ChatId, amount_claimed: WrappedBalance, token_account_id: TokenAccountId) -> bool;
...
fn is_promise_success() -> bool {
    assert_eq!(
        ...
        1,
        "Contact expected a result on the callback"
    );
...
pub enum StorageKey {
    ...
    TipsLookupMap,
    ChatPointsLookupMap,
    ContactDepositsLookupMap,
    ContactTipsLookupMap,
    ContactDeposits,
    ContactUsersInChats,
    WhitelistedTokensLookupSet,
    WhitelistedCategoriesLookupSet,
...
impl NearTips {
    ...
    pub fn new(master_account_id: ValidAccountId,
        ...
        Self {
            deposits: LookupMap::new(StorageKey::ContactDepositsLookupMap),
            tips: LookupMap::new(StorageKey::ContactTipsLookupMap),
            contacts_in_chats: LookupSet::new(StorageKey::ContactUsersInChats),
            ...
            whitelisted_categories: LookupSet::new(StorageKey::WhitelistedCategoriesLookupSet),
            version: 0,
    ...
    pub fn transfer_tips_to_deposit(&mut self, contact: Contact,
                                    account_id: ValidAccountId,
                                    token_id: Option<TokenAccountId>) {
        self.assert_withdraw_available();
        self.assert_check_whitelisted_token(&token_id);
        self.assert_check_whitelisted_category(&contact.category);
        ...
        env::log(format!("@{} transfer {} of {:?} from contact {}:{}. Transfer commission: {} yNEAR",
                         account_id, balance, token_id_unwrapped, contact.category.to_string(), contact.user_id, WITHDRAW_COMMISSION).as_bytes());
    }

    /* SEND TIPS */
    pub fn send_tip_to_contact(&mut self,
                                contact: Contact,
                                amount: WrappedBalance,
                                chat_id: Option<ChatId>,
                                token_id: Option<TokenAccountId>) {
        let account_id = env::predecessor_account_id();
        self.send_tip_to_contact_from_account(account_id, contact.clone(), amount, chat_id, token_id);
    }


    pub(crate) fn send_tip_to_contact_from_account(&mut self,
                ...
                if let Some(chat_settings_unwrapped) = chat_settings {
                    ...
                    if chat_settings_unwrapped.track_chat_points {
                        if amount.0 > MIN_AMOUNT_TO_REWARD_CHAT {
                            let user_in_chat: ContactInChat = ContactInChat {
                                contact: contact.clone(),
                                chat_id: chat_id_unwrapped,
                            };

                            if !self.contacts_in_chats.contains(&user_in_chat) {
                                ...
                                self.contacts_in_chats.insert(&user_in_chat);
                    ...
                    if treasure_fee_numerator > 0 {
                        ...
                        env::log(format!("@{} tipped {} of {:?} for contact {}:{}.",
                                         sender_account_id, tip_amount, token_id_unwrapped, contact.category, contact.user_id).as_bytes());
                    }
                }
            }
        } else {
            env::log(format!("@{} tipped {} of {:?} for contact {}:{}", sender_account_id, tip_amount, token_id_unwrapped, contact.category, contact.user_id).as_bytes());
        }

        // perform a tip
        self.increase_balance(contact.clone(), token_id_unwrapped.clone(), tip_amount);
    ...
    pub fn withdraw_from_contact(&mut self,
                                  contact: Contact,
                                  account_id: ValidAccountId,
        ...
        env::log(format!("@{} is withdrawing {} of {:?} from contact {}:{}",
                         account_id, amount, token_id_unwrapped, contact.category, contact.user_id).as_bytes());
    ...
    pub fn after_ft_transfer_balance(
        ...
        if !is_promise_success() {
            log!("Token {} withdraw by contact {}:{} failed. Amount to recharge: {}",
                 token_account_id, contact.category, contact.user_id, amount.0);

            self.increase_balance(contact.clone(), token_account_id, amount.0);
        }
        promise_success
    }
    ...
    pub fn withdraw_linkdrop(&mut self, public_key: String, contact: Contact) -> Promise {
        self.assert_withdraw_available();
        self.assert_master_account_id();
        // TODO Linkdrop for NOT NEAR???
        let balance: Balance = NearTips::get_balance(self, contact.clone(), Some(NEAR.to_string())).0;
        ...
        self.set_balance_to_zero(contact.clone(), NEAR.to_string());

        Promise::new(self.master_account_id.to_string()).transfer(WITHDRAW_COMMISSION);

        env::log(format!("Contact {}:{} withdrew {} yNEAR with linkDrop for public key {}. Withdraw commission: {} yNEAR",
                         contact.category, contact.user_id, amount, public_key, WITHDRAW_COMMISSION).as_bytes());
    ...
    pub fn get_balances(&self,
        ...
        token_ids
            ...
            .map(|token_account_id|
                (
                    token_account_id.clone(),
                    self.get_balance(contact.clone(), Some(token_account_id.clone())),
                ))
            .collect()
    }

    pub fn whitelist_category(&mut self, category: ContactCategory) {
        self.assert_master_account_id();

        self.whitelisted_categories.insert(&category);
    }

    pub fn is_whitelisted_category(&self, category: ContactCategory) -> bool {
        self.whitelisted_categories.contains(&category)
    }
